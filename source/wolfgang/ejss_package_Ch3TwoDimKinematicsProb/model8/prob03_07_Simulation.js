...
function prob03_07(_topFrame,_libraryPath,_codebasePath, _inputParameters) {
...
var _stringProperties = {};
  ...
  _model.addToReset(function() {
    ...
    __pagesEnabled["Loop Animation"] = true;
  });

  _model.addToReset(function() {
    x = 0.0; // EjsS Model.Variables.Motion Vars.x
    y = 0.0; // EjsS Model.Variables.Motion Vars.y
    vx = 18.0; // EjsS Model.Variables.Motion Vars.vx
    vy = 23.49; // EjsS Model.Variables.Motion Vars.vy
  ...
  _model.addToReset(function() {
    ghostX = new Array(18); // EjsS Model.Variables.Ghost Vars.ghostX
    (function () {
      var _i0;
      for (_i0=0; _i0<18; _i0+=1) {  // EjsS Model.Variables.Ghost Vars.ghostX
        ghostX[_i0] = -100.0;  // EjsS Model.Variables.Ghost Vars.ghostX
      }
    }());
    ghostY = new Array(18); // EjsS Model.Variables.Ghost Vars.ghostY
    (function () {
      var _i0;
      for (_i0=0; _i0<18; _i0+=1) {  // EjsS Model.Variables.Ghost Vars.ghostY
        ghostY[_i0] = 100.0;  // EjsS Model.Variables.Ghost Vars.ghostY
      }
    }());
  ...
  _model.addToInitialization(function() {
    ...
    x = 0.0;  // > Initialization.Init Page:2
    y = 0.0;  // > Initialization.Init Page:3
    vx = 18.0;  // > Initialization.Init Page:4
    vy = 23.49;  // > Initialization.Init Page:5
    // reset ghost images  // > Initialization.Init Page:6
    for (var i=0; i<18; i++) { ghostX[i] = -100.0; ghostY[i] = -100.0; }  // > Initialization.Init Page:7
    next = 0.0;  // > Initialization.Init Page:8
  ...
  function _ODE_evolution1() {
    ...
    __odeSelf.initializeSolver = function() {
      ...
      if (__pagesEnabled["Loop Animation"]) __eventSolver.addEvent(_ODE_evolution1_Event2());
    ...
    __odeSelf.getRate = function(_aState,_aRate) {
      ...
      // Preliminary code: Code to be executed before rate equations are evaluated
        var ay = -2*4.9;  // > Preliminary code for ODE.ODE:1
      // Compute the rate
        var __cRate=0;
        _aRate[__cRate++] = vx; // Rate for ODE: ODE:x
        _aRate[__cRate++] = vy; // Rate for ODE: ODE:y
        _aRate[__cRate++] = ay; // Rate for ODE: ODE:vy
    ...
    _ODE_evolution1_Event1 = function() {
      ...
      // Extract our variables from _aState
        ...
        var vy = _aState[__cOut++];
        var t = _aState[__cOut++];
        var min = 1.0e-5;  // > Event zero-condition for page ODE:1
        if (next - t < min) { min = next - t; }  // > Event zero-condition for page ODE:2
        return min;  // > Event zero-condition for page ODE:3
      };
      ...
      function __userDefinedAction() {
        ghostX[counter] = x;  // > Event action for page ODE:1
        ghostY[counter] = y;  // > Event action for page ODE:2
        next += 0.3;  // > Event action for page ODE:3
        counter++;  // > Event action for page ODE:4
        return false;
      }
    ...
    _ODE_evolution1_Event2 = function() {
      ...
      // Extract our variables from _aState
        ...
        var t = _aState[__cOut++];
        var min = 1.0e-5;  // > Event zero-condition for page ODE:1
        if (5.0 - t < min) { min = 5.0 - t; }  // > Event zero-condition for page ODE:2
        return min;  // > Event zero-condition for page ODE:3
      };
      ...
      function __userDefinedAction() {
        _initialize();  // > Event action for page ODE:1
        return true;
      }
  ...
  function _selectView(_viewNumber) {
    _view = null;
    _view = new prob03_07_View(_topFrame,_viewNumber,_libraryPath,_codebasePath);
...
function prob03_07_View (_topFrame,_viewNumber,_libraryPath,_codebasePath) {
  var _view;
  switch(_viewNumber) {
    case -10 : break; // make Lint happy
    default :
    case 0: _view = prob03_07_View_0 (_topFrame); break;
  } // end of switch
...
function prob03_07_View_0 (_topFrame) {
  ...
  _view._reset = function() {
    ...
    _view._addElement(EJSS_INTERFACE.imageAndTextButton,"titleLabel", _view.title) // EJsS HtmlView.HtmlView Page: declaration of element 'titleLabel'
      .setProperty("Text","<h2> Problem 3.7: A Projectile is Launched </h2>") // EJsS HtmlView.HtmlView Page: setting property 'Text' for element 'titleLabel'
      ;
    ...
    _view._addElement(EJSS_DRAWING2D.drawingPanel,"drawingPanel", _view.plot) // EJsS HtmlView.HtmlView Page: declaration of element 'drawingPanel'
      ...
      .setProperty("MinimumX",-5.5) // EJsS HtmlView.HtmlView Page: setting property 'MinimumX' for element 'drawingPanel'
      ...
      .setProperty("MaximumY",32.0) // EJsS HtmlView.HtmlView Page: setting property 'MaximumY' for element 'drawingPanel'
      .setProperty("MaximumX",89.0) // EJsS HtmlView.HtmlView Page: setting property 'MaximumX' for element 'drawingPanel'
      ;
    ...
    _view._addElement(EJSS_DRAWING2D.shapeSet,"ghostImages", _view.drawingPanel) // EJsS HtmlView.HtmlView Page: declaration of element 'ghostImages'
      .setProperty("NumberOfElements",18) // EJsS HtmlView.HtmlView Page: setting property 'NumberOfElements' for element 'ghostImages'
      .setProperty("FillColor","rgb(239, 191, 190)") // EJsS HtmlView.HtmlView Page: setting property 'FillColor' for element 'ghostImages'
      .setProperty("SizeX",18) // EJsS HtmlView.HtmlView Page: setting property 'SizeX' for element 'ghostImages'
      .setProperty("ShapeType","WHEEL") // EJsS HtmlView.HtmlView Page: setting property 'ShapeType' for element 'ghostImages'
      .setProperty("SizeY",18) // EJsS HtmlView.HtmlView Page: setting property 'SizeY' for element 'ghostImages'
    ...
    _view._addElement(EJSS_DRAWING2D.shape,"projectile", _view.drawingPanel) // EJsS HtmlView.HtmlView Page: declaration of element 'projectile'
      ...
      .setProperty("SizeX",18) // EJsS HtmlView.HtmlView Page: setting property 'SizeX' for element 'projectile'
      .setProperty("SizeY",18) // EJsS HtmlView.HtmlView Page: setting property 'SizeY' for element 'projectile'
    ...
    _view._addElement(EJSS_DRAWING2D.text,"time", _view.drawingPanel) // EJsS HtmlView.HtmlView Page: declaration of element 'time'
      .setProperty("X",-3.8) // EJsS HtmlView.HtmlView Page: setting property 'X' for element 'time'
    ...
    _view._addElement(EJSS_INTERFACE.twoStateButton,"playPause", _view.buttons) // EJsS HtmlView.HtmlView Page: declaration of element 'playPause'
      ...
      .setProperty("Tooltip","Plays/pauses the simulation.") // EJsS HtmlView.HtmlView Page: setting property 'Tooltip' for element 'playPause'
    ...
    _view._addElement(EJSS_INTERFACE.panel,"summary", _view._topFrame) // EJsS HtmlView.HtmlView Page: declaration of element 'summary'
      .setProperty("Html","<p> A projectile is launched as shown in the animation  <strong>(position is given in meters and time is  given in seconds)</strong>. Where does the ball  reach its minimum speed, and what is its speed  when it gets there? </p>") // EJsS HtmlView.HtmlView Page: setting property 'Html' for element 'summary'
      ;
  ...
  return _view;
      ...
      window.addEventListener('load',
        function () { 
          _model =  new prob03_07("_topFrame","_ejs_library/",null);
