...
var accelerometer = EJSS_HARDWARE.accelerometer();
  ...
  var accRunning; // EjsS Model.Variables.Auxiliary Vars.accRunning
  var dataAvailable; // EjsS Model.Variables.Auxiliary Vars.dataAvailable
  ...
  _model._userUnserialize = function(json) {
    ...
    if(typeof json.accRunning != "undefined") accRunning = json.accRunning;
    if(typeof json.dataAvailable != "undefined") dataAvailable = json.dataAvailable;
  ...
  _model.addToReset(function() {
    ...
    accRunning = false; // EjsS Model.Variables.Auxiliary Vars.accRunning
    dataAvailable = false; // EjsS Model.Variables.Auxiliary Vars.dataAvailable
    msg = "Select Sensor to enable accelerometer."; // EjsS Model.Variables.Auxiliary Vars.msg
  });
  ...
  function startSensor() {  // > CustomCode.Sensor Functions:1
    if(accRunning && accelerometer.isPresent()){  // already running and initialized  // > CustomCode.Sensor Functions:2
      var viewData = accelerometer.getViewData();  // > CustomCode.Sensor Functions:3
      accX = viewData.x;  // > CustomCode.Sensor Functions:4
      accY = viewData.y;  // > CustomCode.Sensor Functions:5
      accZ = viewData.z;  // > CustomCode.Sensor Functions:6
      dataAvailable=viewData.x||viewData.y||viewData.z;  // check if all values are zero  // > CustomCode.Sensor Functions:7
      msg=dataAvailable?"":"Accelerometer data not available. Disable sensor simulate motion.";  // > CustomCode.Sensor Functions:8
      if(dataAvailable){ // sensor active  // > CustomCode.Sensor Functions:9
        phi=-Math.atan2(accX,-accY);   //tilt angle  // > CustomCode.Sensor Functions:10
        xCursor=xPivot=0;              // center the pivor  // > CustomCode.Sensor Functions:11
        g=Math.sqrt(accX*accX+accY*accY);  // > CustomCode.Sensor Functions:12
      }  // > CustomCode.Sensor Functions:13
      return;  // > CustomCode.Sensor Functions:14
    }  // > CustomCode.Sensor Functions:15
    if (accelerometer.isPresent()) {  // > CustomCode.Sensor Functions:16
      xCursor=xPivot=0;  // > CustomCode.Sensor Functions:17
      accelerometer.start();  // > CustomCode.Sensor Functions:18
      accelerometer.setAverageInterval(0.1);  // > CustomCode.Sensor Functions:19
      accRunning=true;  // > CustomCode.Sensor Functions:20
      display="none";  // > CustomCode.Sensor Functions:21
      msg="";  // > CustomCode.Sensor Functions:22
    }else{  // > CustomCode.Sensor Functions:23
      display="block";  // > CustomCode.Sensor Functions:24
      accRunning=false;  // > CustomCode.Sensor Functions:25
      dataAvailable=false;  // > CustomCode.Sensor Functions:26
      msg="Accelerometer not available. Use slider to simulate motion.";  // > CustomCode.Sensor Functions:27
    }  // > CustomCode.Sensor Functions:28
  }  // > CustomCode.Sensor Functions:29
  function stopSensor() {  // > CustomCode.Sensor Functions:30
    if (accelerometer.isPresent()) {  // > CustomCode.Sensor Functions:31
      accelerometer.stop();  // > CustomCode.Sensor Functions:32
    }  // > CustomCode.Sensor Functions:33
    g=g0;  // > CustomCode.Sensor Functions:34
    accX=0  // > CustomCode.Sensor Functions:35
    accY=g;  // > CustomCode.Sensor Functions:36
    accZ=0;  // > CustomCode.Sensor Functions:37
    phi=0;  // > CustomCode.Sensor Functions:38
    accRunning=false;  // > CustomCode.Sensor Functions:39
    dataAvailable=false;  // > CustomCode.Sensor Functions:40
    display="block";  // > CustomCode.Sensor Functions:41
  }  // > CustomCode.Sensor Functions:42

  _model.addToInitialization(function() {
    if (!__pagesEnabled["Draw Polar Coord"]) return;
    _view._addOnBlurAction(function () { stopSensor() });  // > Initialization.Draw Polar Coord:1
    var theta=0, dtheta=2*Math.PI/ns;  // > Initialization.Draw Polar Coord:2
    for(var i=0; i<ns; i++){  // > Initialization.Draw Polar Coord:3
      dxSpoke[i]=2*maxR*Math.sin(theta);  // > Initialization.Draw Polar Coord:4
      dySpoke[i]=2*maxR*Math.cos(theta);  // > Initialization.Draw Polar Coord:5
      theta+=dtheta;  // > Initialization.Draw Polar Coord:6
    }  // > Initialization.Draw Polar Coord:7
    var dr=0.5;  // > Initialization.Draw Polar Coord:8
    var r=dr;  // > Initialization.Draw Polar Coord:9
    for(var i=0; i<nr; i++){  // > Initialization.Draw Polar Coord:10
      radii[i]=2*r;  // > Initialization.Draw Polar Coord:11
      r+=dr;  // > Initialization.Draw Polar Coord:12
    }  // > Initialization.Draw Polar Coord:13
  });
  ...
  _model.addToEvolution(function() {
    ...
    dataAvailable=false;  // > Evolution.Read Sensor:1
    if (accRunning) {  // > Evolution.Read Sensor:2
      var viewData = accelerometer.getViewData();  // > Evolution.Read Sensor:3
      accX = viewData.x;  // > Evolution.Read Sensor:4
      accY = viewData.y;  // > Evolution.Read Sensor:5
      accZ = viewData.z;  // > Evolution.Read Sensor:6
      //Assume sensor is not present if all componets are zero  // > Evolution.Read Sensor:7
      dataAvailable=viewData.x||viewData.y||viewData.z;  // > Evolution.Read Sensor:8
      msg=dataAvailable?"":"Accelerometer data not available. Disable sensor to simulate motion.";  // > Evolution.Read Sensor:9
    }  // > Evolution.Read Sensor:10
    if(dataAvailable){ // sensor active  // > Evolution.Read Sensor:11
      phi=-Math.atan2(accX,-accY);   //tilt angle  // > Evolution.Read Sensor:12
      xCursor=xPivot=0;              // center the pivor  // > Evolution.Read Sensor:13
      g=Math.sqrt(accX*accX+accY*accY);  // > Evolution.Read Sensor:14
    }else{   // sensor not available  // > Evolution.Read Sensor:15
      if(display === "none") window.alert("Sensor data not available.");  // > Evolution.Read Sensor:16
      phi=0; // no tilt  // > Evolution.Read Sensor:17
      g=g0;  // > Evolution.Read Sensor:18
      accX=0;  // > Evolution.Read Sensor:19
      accY=g;  // > Evolution.Read Sensor:20
      display="block"; // show slider  // > Evolution.Read Sensor:21
    }  // > Evolution.Read Sensor:22
  });
  ...
  _model.addToFixedRelations(function() {
    ...
    var projection=(dataAvailable && mag>1E-2)?g0*g/mag:g0;  // > FixedRelations.Compute derived variables:7
    ...
    tiltMsg=dataAvailable?"device tilt ="+(phi*180/Math.PI).toFixed(1):"";  // > FixedRelations.Compute derived variables:16
  });
  ...
  function _ODE_evolution1() {
    ...
    __odeSelf.getRate = function(_aState,_aRate) {
      ...
      // Preliminary code: Code to be executed before rate equations are evaluated
        ...
        if(dataAvailable && display==="none"){ // use sensor and proper acceleration  // > Preliminary code for ODE.Pendulum ODE:2
  ...
  function _selectView(_viewNumber) {
    ...
    _view._reset = function() {
      ...
      switch(_viewNumber) {
        ...
        default :
        case 0:
          _view.sensorCheck.linkProperty("Checked",  function() { return display==="none"; } ); // Pendulum View linking property 'Checked' for element 'sensorCheck'
          _view.sensorCheck.setAction("OnCheckOff", function() {
  stopSensor();

}); // Pendulum View setting action 'OnCheckOff' for element 'sensorCheck'
          _view.sensorCheck.setAction("OnCheckOn", function() {
  startSensor ();

}); // Pendulum View setting action 'OnCheckOn' for element 'sensorCheck'
          ...
          _view.sliderPanel.linkProperty("Display",  function() { return display; }, function(_v) { display = _v; } ); // Pendulum View linking property 'Display' for element 'sliderPanel'
  ...
  b=Math.min(1,b);
          ...
          _view.runPauseButton.setAction("OnClick", _play); // Pendulum View setting action 'OnClick' for element 'runPauseButton'
...
function PendulumWithMovingSupport_View_0 (_topFrame) {
  ...
  _view._reset = function() {
    ...
    _view._addElement(EJSS_INTERFACE.checkBox,"sensorCheck", _view.checkPanel) // EJsS HtmlView.Pendulum View: declaration of element 'sensorCheck'
      .setProperty("Text","Sensor") // EJsS HtmlView.Pendulum View: setting property 'Text' for element 'sensorCheck'
      ;
    ...
    _view._addElement(EJSS_INTERFACE.panel,"sliderPanel", _view.topPanel) // EJsS HtmlView.Pendulum View: declaration of element 'sliderPanel'
      .setProperty("CSS",{"display":"block"}) // EJsS HtmlView.Pendulum View: setting property 'CSS' for element 'sliderPanel'
      ;
    ...
    _view._addElement(EJSS_DRAWING2D.segment,"armSegment", _view.pendulumGroup) // EJsS HtmlView.Pendulum View: declaration of element 'armSegment'
      ...
      .setProperty("LineColor","DarkGray") // EJsS HtmlView.Pendulum View: setting property 'LineColor' for element 'armSegment'
